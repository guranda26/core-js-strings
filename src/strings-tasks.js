...
function removeFirstOccurrences(str, value) {
  ...
  if (index !== -1) {
    const beforeIn = str.slice(0, index);
    const afterIn = str.slice(index + value.length);
    return beforeIn + afterIn;
  }
...
function removeLastOccurrences(str, value) {
  ...
  if (lastInd !== -1) {
    const beforeIn = str.slice(0, lastInd);
    const afterIn = str.slice(lastInd + value.length);
    return beforeIn + afterIn;
  }
...
function formatTime(minutes, seconds) {
  ...
  const formatSecs = String(seconds).padStart(2, '0');
...
function extractNameFromTemplate(value) {
  const matchWord = value.match(/Hello, (.+?)!/i);
  return matchWord ? matchWord[1] : value;
}
...
function extractEmails(str) {
  ...
  const emails = emailArray.filter((email) =>
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    ...
    const baseCharCode = upper ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
    const offset = (char.charCodeAt(0) - baseCharCode + 13) % 26;
    const newCharCode = baseCharCode + offset;
...
function getCardId(value) {
  ...
  const rankIndex =
    ranks.indexOf(rank) !== -1 ? ranks.indexOf(rank) : ranks.indexOf('X');
